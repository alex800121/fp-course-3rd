...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  f <$> StateT a = StateT $ \s -> let x = a s in (\(m, n) -> (f m, n)) <$> x 
...
instance Monad f => Applicative (StateT s f) where
  ...
  StateT f <*> StateT a =
    StateT $ \s ->
    f s >>= \(f', sf) -> a sf >>= \(a', sa) -> return (f' a', sa)
...
runState' ::
  ...
  -> s
  -> (a, s)
runState' (StateT a) s = let ExactlyOne x = a s in x
...
distinctF l = let
  ...
  in evalT (filtering f l) S.empty
...
instance Monad f => Applicative (OptionalT f) where
  ...
  OptionalT f <*> OptionalT a = OptionalT $
    f >>= \f' -> case f' of
      Empty   -> pure Empty
      Full fx -> a >>= \a' -> pure (fx <$> a')
...
distinctG l = let
  f = \x -> StateT $ \s -> OptionalT $ if x > 100
    then log1 (listh ("aborting > 100: " P.++ show x)) Empty
    else if even x
      then log1 (listh ("even number: " P.++ show x)) (Full (S.notMember x s, S.insert x s))
      else pure (Full (S.notMember x s, S.insert x s))
  in runOptionalT $ evalT (filtering f l) S.empty

-- StateT S.Set (OptionalT (Logger Chars)) Bool
-- StateT ::  (S.Set -> OptionalT (Logger Chars) (Bool, S.Set))
--            (S.Set -> OptionalT ((Logger Chars) (Optional (Bool, S.Set))))
--            (S.Set -> OptionalT ((Logger (List Chars) (Optional (Bool, S.Set))))
