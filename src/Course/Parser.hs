...
(|||) ::
  ...
  -> Parser a
  -> Parser a
P fa ||| P fb = P $ \s -> case isErrorResult (fa s) of
  True  -> fb s
  _     -> fa s
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  f =<< P fa = P $ \s -> onResult (fa s) (\i a -> parse (f a) i)
...
list ::
  Parser a
  -> Parser (List a)
list a = (lift2 (:.) a (list a)) ||| pure Nil
...
list1 ::
  Parser a
  -> Parser (List a)
list1 a = a >>= \a' -> lift2 (:.) (pure a') (list a)
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = character >>= \c -> if f c then return c else unexpectedCharParser c
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\x acc -> lift2 (:.) x acc) (pure Nil) 
...
surnameParser ::
  Parser Chars
surnameParser = upper >>= \a -> thisMany 5 lower >>= \as -> list lower >>= \x -> pure (a :. as ++ x)
...
phoneParser ::
  Parser Chars
phoneParser = digit >>= \a -> phoneBodyParser >>= \as -> is '#' >> pure (a :. as)
