...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft f (ListZipper l a r) = let (x, y) = break f l in case y of
  Nil       -> IsNotZ
  (z :. zs) -> IsZ $ ListZipper zs z (reverse x ++ a :. r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight f (ListZipper l a r) = let (x, y) = break f r in case y of
  Nil       -> IsNotZ
  (z :. zs) -> IsZ $ ListZipper (reverse x ++ a :. l) z zs
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper l a r) = case l of
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l a r) = case r of
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper l a r) = case l of
  Nil       -> IsNotZ
  (x :. xs) -> IsZ $ ListZipper xs x (a :. r)
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper l a r) = case r of
  Nil       -> IsNotZ
  (x :. xs) -> IsZ $ ListZipper (a :. l) x xs
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper l a r) = case l of
  Nil       -> IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper l a r) = case r of
  Nil       -> IsNotZ
...
moveLeftN n lz
  ...
  | otherwise = foldRight ($) (IsZ lz) (replicate n (moveLeft -<<))
...
moveRightN n lz
  ...
  | otherwise = foldRight ($) (IsZ lz) (replicate n (moveRight -<<))
...
instance Extend ListZipper where
  f <<= lz = ListZipper l' (f lz) r'
    where
      l' = f <$> unfoldr (g . moveLeft) lz
      r' = f <$> unfoldr (g . moveRight) lz
      g = ((\x -> (x, x)) <$>) <$> toOptional
